...
from torch_geometric.nn import GatedGraphConv,GCNConv,TopKPooling,GlobalAttention
...
class GCN_simplify2(nn.Module):
    ...
    def forward(self, x, edge_index):
        x = x.to(torch.device("cuda:0"))
        edge_index = edge_index.to(torch.device("cuda:0"))
        post_conv = self.relu(self.conv1(x, edge_index))
        post_conv = self.dropout(post_conv)
        post_conv = self.connect(post_conv)
        post_conv = self.relu(self.conv2(post_conv,edge_index))
        post_conv = self.conv3(post_conv,edge_index)
        pooled = global_max_pool(post_conv, torch.zeros(post_conv.shape[0], dtype=int, device=post_conv.device))
        y_a = self.__classifier(pooled)
        result = nn.Softmax(dim=1)(y_a)
        return result
...
class GCN_simplify(nn.Module):
    ...
    def __init__(self, output_dim=200, input_dim=100):
        ...
        #         #[nn.ReLU(), nn.Dropout(0.3), nn.Linear(output_dim, 2)]
        #         [nn.Dropout(0.3), nn.Linear(output_dim, 2)]
        # ))
    ...
    def forward(self, x, edge_index):
        ...
        #post_conv = de_batchify_graphs(post_conv,batch)

        feature_vec = x.to(torch.device("cuda:0"))
        edge_index = edge_index.to(torch.device("cuda:0"))
        for conv, relu in zip(self.convs, self.relus):
        ...
        # pooled = self.readout(post_conv, torch.zeros(post_conv.shape[0], dtype=int, device=post_conv.device)).squeeze(dim=1)
        # #out = self.ffn(pooled)
...
class GraphConvEncoder(torch.nn.Module):
    ...
    def __init__(self, output_dim=200, input_dim=100):
        ...
        self.input_GCL = GCNConv(input_dim, output_dim)
    ...
    def forward(self,x, edge_index):
        ...
        batch = torch.zeros(node_embedding.shape[0], dtype=int, device=node_embedding.device)
...
class DeepWukong(nn.Module):
    def __init__(self, output_dim=200, input_dim=100):
        ...
        # self.readout = TopKPooling(output_dim, ratio=0.8)
        # self.attnpool = GlobalAttention(torch.nn.Linear(output_dim, 1))
        #self.softmax = nn.Softmax(dim=1)
        #self.softmax = nn.Sigmoid()
    def forward(self, x, edge_index):
        x = x.to(torch.device("cuda:0"))
        ...
        # node_embedding, edge_index, _, batch, _, _ = self.readout(outputs,edge_index,None,torch.zeros(outputs.shape[0], dtype=int, device=outputs.device))
        # pooled = self.attnpool(node_embedding,torch.zeros(node_embedding.shape[0], dtype=int, device=node_embedding.device))
...
class DeepWukong2(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        # node_embedding, edge_index, _, batch, _, _ = self.readout(outputs,edge_index,None,torch.zeros(outputs.shape[0], dtype=int, device=outputs.device))
        # pooled = self.attnpool(node_embedding,torch.zeros(node_embedding.shape[0], dtype=int, device=node_embedding.device))
        pooled = global_add_pool(outputs, torch.zeros(outputs.shape[0], dtype=int, device=outputs.device))
...
class IVDetect(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        result = nn.Softmax(dim=1)(pooled)
        return result
